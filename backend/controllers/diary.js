...
const saveDiary = async (req, res, next) => {
    try {
        ...
        const userId = req.user.id;
        const mlResult = await analyzeText(content);
        ...
        if (content && content.trim().length > 5) {
            ...
            try {
                ...
                updateData.emotion = analysis.emotion;
                updateData.reason = analysis.primaryReason;
                updateData.riskLevel = analysis.riskLevel;
                updateData.recommendations = analysis.recommendations;

            } catch (mlError) {

                console.log("ML ERROR:", mlError.message);
